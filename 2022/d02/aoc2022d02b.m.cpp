...
namespace {
    ...
    constexpr int myScore(char wantOutcome, char otherShape) noexcept {
        Shape myFig;
        Shape otherFig = isFig(otherShape);
        if (wantOutcome == 'X')  // lose
        {
            if (otherFig == Rock) myFig = Scissors;
            else if (otherFig == Paper) myFig = Rock;
            else /*if (otherFig == Scissors)*/ myFig = Paper;
        } else if (wantOutcome == 'Y') // draw
        {
            myFig = otherFig;
        } else /*if (wantOutcome == 'Z')*/ // win
        {
            if (otherFig == Rock) myFig = Paper;
            else if (otherFig == Paper) myFig = Scissors;
            else /*if (otherFig == Scissors)*/ myFig = Rock;
        }
        // tie
