...
int fuelForMass(int mass) noexcept { return mass / 3 - 2; }

int calculateTotalFuel(int mass) noexcept {
  int totalFuel = 0;
  for (;;) {
    int const fuel = fuelForMass(mass);
    if (fuel <= 0) {
      return totalFuel;
    }
    totalFuel += fuel;
...
struct Module {
  ...
  [[nodiscard]] int fuelNeeded() const noexcept {
    return calculateTotalFuel(mass);
  }
