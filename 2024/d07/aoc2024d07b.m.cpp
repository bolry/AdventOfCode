...
struct Equation {
  ...
  bool isPossible() const {
    ...
    if (rest.testValue >= 0 && rest.isPossible())
      return true;
    rest.testValue = testValue / nums.back();
    if (testValue % nums.back() == 0 && rest.isPossible())
      return true;
    auto valueStr = std::to_string(testValue);
    auto suffix = std::to_string(nums.back());
    if (not valueStr.ends_with(suffix))
      return false;
    valueStr.resize(valueStr.size() - suffix.size());
    if (valueStr.empty())
      return false;
    rest.testValue = std::stoll(valueStr);
    return rest.isPossible();
  }
...
void solve2024day7b(std::istream &input) {
