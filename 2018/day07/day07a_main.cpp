...
try {
  ...
  while(!instructions.empty()){
    ...
    instructions.erase(std::remove_if(instructions.begin(), instructions.end(), [&next](Instruct const& in){return in.before == next;}), instructions.end());
  }
  std::cout << "Day 7 A result " << '\n';
  return EXIT_SUCCESS;
}
