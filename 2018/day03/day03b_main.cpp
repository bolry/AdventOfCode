...
bool only_claimer(Claim const &claim, std::vector<Row> const &fabric) {
  int const col_first = claim.left_offset;
  int const col_last = col_first + claim.width;
  for (int i = claim.top_offset; i < claim.top_offset + claim.height; i++) {
    auto row_begin = fabric.at(i).begin();
    if (std::any_of(row_begin + col_first, row_begin + col_last, [](int const n) { return n > 1; }))
      return false;
  }
  return true;
}
...
try {
  ...
  for (Claim const &claim : claims) {
    if (only_claimer(claim, fabric)) {
      std::cout << "Single claim ID is " << claim.id << '\n' << std::flush;
      return EXIT_SUCCESS;
    }
  }
  std::cout << "Oops, didn't find any single claim\n" << std::flush;
  return EXIT_FAILURE;
}
